...
pub fn render(spec: &ApiSpec) -> String {
    ...
    writeln!(out, "//! `{}` client crate, generated by `oxide-gen`.", spec.display_name).unwrap();
    ...
    writeln!(out, "//!").unwrap();
    writeln!(out, "//! Do not edit by hand — re-run `oxide-gen` to regenerate.").unwrap();
    writeln!(out).unwrap();
...
fn render_client(out: &mut String, spec: &ApiSpec) {
    ...
    writeln!(out, "    }}").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "    /// Build a client pointing at the default endpoint `{default_base}`.").unwrap();
...
fn render_rest_body(out: &mut String, _spec: &ApiSpec, op: &Operation) {
    ...
    for p in &path_params {
        url_template = url_template.replace(&format!("{{{}}}", p.original_name), &format!("{{{}}}", p.name));
    }
    write!(out, "        let url = format!(\"{{base}}{}\"", url_template).unwrap();
    ...
    // Query params.
    for p in op.params.iter().filter(|p| p.location == ParamLocation::Query) {
        if p.required {
            writeln!(
                out,
    ...
    // Header params.
    for p in op.params.iter().filter(|p| p.location == ParamLocation::Header) {
        if p.required {
            writeln!(
                out,
    ...
    writeln!(out, "        let response = req.send().await?.error_for_status()?;").unwrap();
    writeln!(
        out,
